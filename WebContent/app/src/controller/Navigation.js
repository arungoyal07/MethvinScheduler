Ext.define("methwin.client.controller.Navigation", {
	extend : 'Ext.app.Controller',

	views : [ 'Navigation' ],

	refs : [
	// This auto-generates a "getEmployeeScheduler" getter for this
	...
	getResourceScheduler : function() {
		if (!this.scheduler) {
			...
			this.scheduler = new methwin.client.view.ResourceSchedule({
			...
			gantt.on('zoomchange', function() {
				...
				this.scheduler.zonesPlugin.setDisabled(Sch.util.Date
						.compareUnits(this.scheduler.timeAxis.unit,
								Sch.util.Date.WEEK) > 0);
			}, this);
		}
	...
	getHistogram : function() {
		if (!this.histogram) {
			...
			this.histogram = new methwin.client.view.ResourceHistogram({
				partnerTimelinePanel : gantt,
